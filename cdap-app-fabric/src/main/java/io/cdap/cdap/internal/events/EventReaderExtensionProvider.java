...
public class EventReaderExtensionProvider extends AbstractExtensionLoader<String, EventReader>
  ...
  protected Set<String> getSupportedTypesForProvider(EventReader eventReader) {
    if (enabledEventReaders == null || !enabledEventReaders.contains(eventReader.getClass().getName())) {
      LOG.debug("{} is not present in the enabled list of event writers.", eventReader.getClass().getName());
